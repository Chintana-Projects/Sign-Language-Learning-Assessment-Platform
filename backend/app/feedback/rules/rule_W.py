...
class RuleW(GestureRule):
    ...
    def evaluate(self, landmarks, deviations, messages):
        ...
        if landmarks[16][1] > landmarks[14][1]:
            deviations.append("Ring finger should be extended.")

        if landmarks[20][1] < landmarks[18][1]:
            deviations.append("Pinky finger should be folded.")

        if deviations:
            messages.append("Extend your index, middle and ring fingers.")
            return False

        messages.append("W gesture looks correct.")
        return True
...
class RuleU(GestureRule):
    ...
    def evaluate(self, landmarks, deviations, messages):
        ...
        if abs(landmarks[8][0] - landmarks[12][0]) > 0.04:
            deviations.append("Index and middle fingers should stay together.")

        if deviations:
            messages.append("Keep the index and middle fingers together while folding the others.")
            return False

        messages.append("U gesture looks correct.")
        return True